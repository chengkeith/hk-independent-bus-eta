...
const getAllRouteStopsBearings = (routeListParams : SearchParameters) => {
  const { routeStops , routeList , stopList } = routeListParams;
...
const searchNearbyStops = (targetStopId : string, excludedStopIdList : string[], searchParams : SearchParameters) => {
  const { routeList , stopList } = searchParams;
  const bearingTargets = getAllRouteStopsBearings(searchParams);
  ...
  .reduce( (acc, stopId) => {
    ...
    const _searchParams : SearchParameters = { routeList, stopList, routeStops : _routeStops };
    const bearings = getAllRouteStopsBearings(_searchParams);
...
export const useStopGroup = ({
  ...
  const searchParams : SearchParameters = { routeList, stopList, routeStops };
  ...
  stopKeys.forEach((stopKey) => {
    ...
    stopListEntries = stopListEntries.concat(searchNearbyStops(stopId, stopListEntries.map((stop) => stop.id), searchParams));
    for(let i = 0; i < stopListEntries.length; ++i) { // use traditional for-loop as the length keeps expanding
      stopListEntries = stopListEntries.concat(searchNearbyStops(stopListEntries[i].id, stopListEntries.map((stop) => stop.id), searchParams));
